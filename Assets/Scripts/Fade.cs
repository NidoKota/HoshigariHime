    ...
    static IEnumerator Fe<T>(IObserver<Unit> observer, T fadeObject, float speed, float firstAlfa, float endAlfa, bool unscaledTime, bool saveDisposable)
    {
        ...
        while (true)
        {
            try
            {
                ...
                else if (typeof(T) == typeof(WipeCircle)) (fadeObject as WipeCircle).radius.value = Mathf.Lerp(endAlfa, firstAlfa, t);
                else if (typeof(T) == typeof(ColorAdjustments)) (fadeObject as ColorAdjustments).colorFilter.value = new Color(Mathf.Lerp(endAlfa, firstAlfa, t), Mathf.Lerp(endAlfa, firstAlfa, t), Mathf.Lerp(endAlfa, firstAlfa, t), 0);
