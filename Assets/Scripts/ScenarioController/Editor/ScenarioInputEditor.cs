...
namespace ScenarioController.Editor
{
    ...
    public class ScenarioInputDrawer : PropertyDrawer
    {
        ...
        /// </summary>
        float GetScenariosReorderableListHeight(SerializedProperty property)
        {
            float height = 40;
            int arraySize = property.FindPropertyRelative("scenarios").arraySize;

            if (arraySize == 0) height += 80;
            else for (int i = 0; i < arraySize; i++) height += 80;
            return height;
        }
        ...
        {
            return property.FindPropertyRelative("display").boolValue ? GetScenariosReorderableListHeight(property) + (tools ? 113 : 53) + (isPlayableAsset ? 0 : 20) : 32;
        }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            using (new EditorGUI.PropertyScope(position, label, property))
            {
                ...
                if (!once)
                {
                    isPlayableAsset = property.serializedObject.targetObject is UnityEngine.Playables.PlayableAsset;
                ...
                if (displayProp.boolValue)
                {
                    ...
                    Rect scenariosReorderableListRect = new Rect()
                    {
                        ...
                        xMax = position.xMax,
                        yMin = propertyHight,
                        height = GetScenariosReorderableListHeight(property)
                    };
                    
                    //scenariosReorderableList描画
                    scenariosReorderableList.DoList(scenariosReorderableListRect);
                    propertyHight += GetScenariosReorderableListHeight(property) + 2;
